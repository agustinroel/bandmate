...
export async function getSongByIdForUser(userId, id) {
    const { data, error } = await supabase
        .from("songs")
        .select("*")
        .eq("id", id)
    ...
    if (data.is_seed === true)
        return mapSong(data);
    if (data.owner_id === userId)
        return mapSong(data);
    return null;
}
export async function createSongForUser(userId, dto) {
    const payload = {
...
export async function updateSongForUser(userId, id, patch) {
    ...
    if (p.durationSec !== undefined)
        payload.duration_sec = p.durationSec ?? null;
    if (p.notes !== undefined)
        payload.notes = p.notes ?? null;
    // --- BODY ---
    if (p.sections !== undefined)
        payload.sections = p.sections ?? [];
    // version: si viene, la seteamos; si no viene no tocamos
    if (p.version !== undefined)
        payload.version = p.version ?? existing.version ?? 1;
    // --- EXTERNAL IDs ---
    if (p.spotify_id !== undefined)
        payload.spotify_id = p.spotify_id ?? null;
    if (p.spotifyId !== undefined)
        payload.spotify_id = p.spotifyId ?? null;
    if (p.musicbrainz_id !== undefined)
        payload.musicbrainz_id = p.musicbrainz_id ?? null;
    if (p.musicbrainzId !== undefined)
        payload.musicbrainz_id = p.musicbrainzId ?? null;
    if (p.is_imported !== undefined)
        payload.is_imported = p.is_imported;
    if (p.isImported !== undefined)
        payload.is_imported = p.isImported;
    // --- LINKS ---
...
export async function deleteSongForUser(userId, id) {
    const { data: existing, error: e1 } = await supabase
        .from("songs")
...
export async function rateSongForUser(userId, id, rating) {
