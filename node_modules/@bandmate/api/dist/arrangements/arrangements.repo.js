...
export async function createArrangementForWork(userId, workId, dto) {
    // validar work existe
...
export async function getArrangementById(arrangementId) {
    const { data, error } = await supabase
        .from("arrangements")
        .select("*")
        .eq("id", arrangementId)
...
export async function updateArrangementForUser(userId, arrangementId, patch) {
    ...
    if (Object.keys(payload).length === 0)
        return mapArrangement(existing);
    const { data, error } = await supabase
        .from("arrangements")
        .update(payload)
        .eq("id", arrangementId)
...
export async function deleteArrangementForUser(userId, arrangementId) {
    const { data: existing, error: e1 } = await supabase
        .from("arrangements")
...
export async function rateArrangementForUser(userId, arrangementId, rating) {
