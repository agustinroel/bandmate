...
function pickSeedSlug(rightsNotes: any): string | null {
...
export async function listLibrary() {
  ...
  const bestByWork = new Map<string, any>();
  for (const a of arrs ?? []) {
    const prev = bestByWork.get((a as any).work_id);
    if (!prev) bestByWork.set((a as any).work_id, a);
    else {
      // mejor por rating_count primero, luego rating_avg
      const pc = (prev.rating_count ?? 0) as number;
      const nc = ((a as any).rating_count ?? 0) as number;
      const pa = (prev.rating_avg ?? 0) as number;
      const na = ((a as any).rating_avg ?? 0) as number;

      const better =
        nc > pc ||
        (nc === pc && na > pa) ||
        (nc === pc && na === pa && (a as any).updated_at > prev.updated_at);

      if (better) bestByWork.set((a as any).work_id, a);
    }
  }
...
export async function getLibraryWork(workId: string) {
