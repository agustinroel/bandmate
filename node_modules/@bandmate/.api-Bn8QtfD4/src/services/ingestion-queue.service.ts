...
export async function addToIngestionQueue(data: any) {
  ...
  processFallback(data).catch((err) =>
...
async function processArtistIngestion(artistName: string, userId: string) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`MB API error: ${response.status}`);
    const data = await response.json();
    const recordings = data.recordings || [];

    console.log(
      `[Ingestion] Found ${recordings.length} recordings for ${artistName}.`,
    );
    ...
    for (const rec of recordings) {
      ...
      if (useFallback) await new Promise((r) => setTimeout(r, 1000));
    }
...
async function processSingleSongIngestion(mbid: string, userId: string) {
  console.log(`[Ingestion] Processing song MBID: ${mbid}`);

  try {
    const details = await musicBrainzService.getRecordingDetails(mbid);
    if (!details) throw new Error("Metadata not found");
    ...
    console.log(
      `[Ingestion] Successfully ingested: ${work.title} - ${work.artist}`,
    );
