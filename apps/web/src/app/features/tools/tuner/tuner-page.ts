      ...
      <div class="tuner-card">
        ...
        <div class="controls">
          @if (!isListening()) {
            ...
            <button mat-stroked-button (click)="stopTuner()" class="stop-btn">Stop</button>
          }
        </div>
      </div>
  ...
  styles: [
    `
      ...
      .tuner-header h1 {
        ...
        animation: bmFadeInUp 0.6s var(--bm-ease-out) both;
      }
      ...
      .subtitle {
        ...
        letter-spacing: 2px;
        margin-top: 8px;
        animation: bmFadeInUp 0.8s var(--bm-ease-out) both;
      ...
      .tuner-card {
        ...
        /* Entrance Animation */
        animation: bmScaleIn 0.7s var(--bm-ease-out) both;
      }
...
export class TunerPageComponent implements OnDestroy {
  ...
  async startTuner() {
    this.achievements.unlock('tuner_used_first');
    try {
  ...
  private autoCorrelate(buf: Float32Array, sampleRate: number): number {
    ...
    for (let i = 0; i < newSize; i++)
      for (let j = 0; j < newSize - i; j++) c[i] = c[i] + subBuf[j] * subBuf[j + i];
