...
export class AIOrchestratorService {
    genAI = null;
    model = null;
    currentModelName = "gemini-2.0-flash"; // Default
    ...
    async generateForWork(mbid) {
        ...
        for (const modelName of this.MODEL_PRIORITY) {
            try {
                console.log(`[AI-Orchestrator] Attempting generation with ${modelName}...`);
                ...
                lastError = err;
                console.warn(`[AI-Orchestrator] Model ${modelName} failed:`, err.message || err);
    ...
    async realCallLLM(model, title, artist) {
        const prompt = `
    ...
    async mockFetchLyrics(title, artist) {
        return "Sample lyrics...";
    }
    async callLLM(title, artist, lyrics) {
