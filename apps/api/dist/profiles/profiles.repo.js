...
export async function listPublicProfiles(filters = {}, limit = 50, offset = 0) {
    let query = supabase
        .from("profiles")
        .select("id, username, full_name, avatar_url, instruments, genres, sings, bio, is_public")
        .eq("is_public", true);
    if (filters.query) {
    ...
    if (error)
        throw error;
    return data; // relaxed type for now
}
