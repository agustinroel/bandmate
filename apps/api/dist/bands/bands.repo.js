...
export async function createBand(userId, data) {
    // 1. Create Band
...
export async function getBand(bandId) {
    const { data, error } = await supabase
        .from("bands")
        .select("*")
        .eq("id", bandId)
...
export async function listBandSongs(bandId) {
    const { data, error } = await supabase
        .from("band_songs")
        .select(`
      shared_at,
...
export async function addSongToBand(userId, bandId, songId) {
...
export async function removeSongFromBand(userId, bandId, songId) {
...
export async function updateBand(bandId, data) {
...
export async function getOrCreateInviteCode(bandId) {
...
export async function joinBandByCode(userId, code) {
...
export async function inviteUserToBand(bandId, invitedBy, userIdToInvite) {
    ...
    await createNotification(userIdToInvite, "band_invite", "Band Invitation", `You have been invited to join ${bandName}`, { bandId, bandName, inviteId: newInvite.id });
    return { success: true };
}
