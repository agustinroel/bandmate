import { addSongToBand, createBand, getBand, listBandMembers, listBandSongs, listBandsForUser, removeSongFromBand, updateBand, getOrCreateInviteCode, joinBandByCode, inviteUserToBand, listPendingInvites, respondToInvite, } from "./bands.repo.js";
export async function bandsRoutes(app) {
    ...
    // Create a new band
    app.post("/bands", async (req, reply) => {
        const user = req.user;
        if (!user)
            return reply.code(401).send({ message: "Unauthorized" });
        const body = req.body;
    ...
    app.get("/bands/:id", async (req, reply) => {
        ...
        try {
            ...
            if (!band)
                return reply.code(404).send({ message: "Band not found" });
            return band;
        }
        catch (e) {
            req.log.error(e);
    ...
    app.patch("/bands/:id", async (req, reply) => {
        const { id } = req.params;
        const body = req.body;
        try {
